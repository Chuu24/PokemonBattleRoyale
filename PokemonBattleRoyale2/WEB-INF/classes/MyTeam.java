...
public class MyTeam extends HttpServlet{
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response){
        try{
            ...
            List<TeamPokemon> notMainTeam = new ArrayList<>();
            ...
            while(rs.next()){
                ...
                myPokes.add(new CatchedPokemon(idPoke, idTrai, nivel, nickname));
            }
            ...
            while(rs1.next()){
                ...
                if(principal == 1){
                    ...
                    notMainTeam.add(new TeamPokemon(idPoke, idTrai, nivel, nickname, imagen));
                }
            }
            
            request.setAttribute("myPokes", myPokes);
            request.setAttribute("mainTeam", mainTeam);
            request.setAttribute("fakeTeam", notMainTeam);
            
            stat.close();
            stat1.close();
            con.close();

            RequestDispatcher disp =  getServletContext().getRequestDispatcher("/myteam.jsp");
